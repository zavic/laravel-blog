<x-app-layout>
    ...
    <div class="flex mt-4">
        <a href="#" class="px-1 text-sm">All</a>
        <a href="#" class="px-1 text-sm">|</a>
        <a href="#" class="px-1 text-sm">Published</a>
        <a href="#" class="px-1 text-sm">|</a>
        <a href="#" class="px-1 text-sm">Draft</a>
        <a href="#" class="px-1 text-sm">|</a>
        <a href="#" class="px-1 text-sm">Trash</a>
    </div>
            ...
            <tbody class="bg-white dark:bg-slate-800">
                @forelse ($posts as $item)
                    <tr
                        ...
                        <td class="px-2 py-4">
                            <a href="{{ route('posts.edit', $item->id) }}">{{ $item->title }}</a>
                        </td>
                        <td class="px-2 py-4">
                        ...
                        <td class="px-2 py-4">
                            @if ($item->published_at === null)
                                <span class="px-3 py-1.5 bg-gray-500 text-white text-xs rounded-full">Draft</span>
                            @elseif ($item->published_at > now())
                                <p class="text-green-600">{{ date('F jS, Y', strtotime($item->published_at)) }}</p>
                            @else
                                <p class="">{{ date('F jS, Y', strtotime($item->published_at)) }}</p>
                            @endif
                        </td>
                        <td class="py-4">
                            <div class="flex">
                                ...
                                <a href="{{ route('posts.destroy', $item->id) }}" data-confirm-delete="true">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor"
                                        class="size-6 hover:text-blue-400 me-2">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-8">No post yet</td>
                    </tr>
                @endforelse
